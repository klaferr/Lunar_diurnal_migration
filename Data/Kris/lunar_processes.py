...
def ballistic(temp, i_lat, i_long, i_tod, direction, launch, pMass, vel_dist):
    # this uses teh weighted average for particle velocity 
...
def desorption_R21(T, Ed):
    ...
    A = 10**13                          # 1/s, this may vary. 
    kb_eV = kb/(1.60218*10**(-19))      # Change to eV
...
def surftime(Temperature, molecule):
    # Molecule should be in g/mol
    # desorption
    Ed = 0.6                            # eV
    desorp = desorption_R21(Temperature, Ed)

    f = 0.01                            # percent of a monolayer
    theta_mon = (10**19 * f)/Avo/(molecule/1000)
    tau_surf = theta_mon / desorp
...
def Model_MonteCarlo(particle, dt, t, local_noon, molecule):
    ...
    for i in range(0, int(t/dt), 1):
        ...
        tau_surf = surftime(results[3, i], mole)  
                ...
                if conda == True:
                    ...
                    if condc == True:
                        ...
                        if condb == True:
                            ...
                            tau_surf = surftime(results[3, i+1], mole)
        ...
        if conda == True or condb == True or condc == True:
            ...
            #print('Moon rotates')
            local_noon += (-(360/(24/dt)))%360 # allow loop to run through multiple days
...
def Model_MonteCarlo_produce(particle, dt, t, ms, on, local_noon, molecule):
    ...
    results[:n, 4, 0] = False #np.zeros((np.size(particle[:, 0])))
    ...
    for i in range(0, int(t/dt)-1, 1):
        ...
        for nn in range(0, ln, 1):
            ...
            if results[nn, 4, i] == 1 or np.isnan(results[nn, 4, i]):
                ...
                # define how long it sits for
                tau_surf = surftime(results[nn, 3, i], mole)  
                ...
                if tau_surf >= lunar_dt:
                    ...
                    while tot_time < lunar_dt:
                        #print(tot_time, lunar_dt)
                        # while the time of jumping is less than a timestep
                        conda = loss(sigma_Sputtering, photo_lifespan, tau_surf, jump_hold[2], cosi)
                        if conda == True:
                            #print('Particle is lost during sitting, following a jump')
                            # if lost to loss mechanism, exit loop. 
                            results[nn, 4, i+1] = conda
                            ...
                            if condc == True:
                                ...
                                if condb == True:
                                    ...
                                    tau_surf = surftime(jump_hold[3], mole)
        ...
        if n+ms*i > on+m:
            ...
            print(results[n+ms*i:n+ms*(i+1), 2, i+1])

            results[n+ms*i:n+ms*(i+1), 4, i+1] = np.zeros((ms))
            print(n+ms*i, n+ms*(i+1))
...
# for cluster
# path_to_file = '/home/klaferri/Desktop/Research/Lunar_diurnal_migration/Date/Kris/' 
