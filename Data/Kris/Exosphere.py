...
def main():
    ...
    p.add_argument("-tt",default=250,type=float,help="Jumps per step max")
    ...
    sub = int(np.size(particles[:, 0])/10)
    ...
    results = np.zeros((nn, 9, tt))*np.nan
    ...
    for n in range(0, nn, 1):
        ...
        results[n, :, :] = exosphere_multiple(particles[n, :], tt, dt, t, local_noon, molecule)
        if n % 10 ==0:
            sys.stderr.flush()  
    ...
    #fheader = "latitude, longitude, time of day, temperature, condition, launch angle, height, velocity, time of flight,"

    # save as .npy file
    filename = directory + 'Exosphere_smooth_p' +str(nn) + '_t' + str(int(t*dt)) + '_' + molecule + '_i' + str(int(segment)) + '.npy'
...
def exosphere_multiple(particle, tt, dt, t, local_noon, nn, molecule):
    if molecule == "H2O":
        pMass = pr.mass_H2O
        mole = pr.m_H2O
    ...
    for i in range(0, tt-1, 1):
        ...
        tau_surf = pr.surftime(out[3, i], mole)  
        ...
        if tau_surf >= lunar_dt:
            ...
            cosi = pr.cosi_smooth(out[0, i], 0, np.deg2rad(15*out[2, i])-np.pi)
